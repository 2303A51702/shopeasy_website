...
export default function DeliverySignup() {
  ...
  const handleSubmit = async e => {
    ...
    setError('');
    try {
      const res = await api.post('/delivery/signup', form);
  ...
  return (
    <div style={s.wrap}>
      <div style={s.box}>
        <h2 style={s.title}>🚚 Join as Delivery Partner</h2>
        <p style={s.sub}>Create your delivery partner account</p>
        {error && <p style={s.err}>{error}</p>}
        <form onSubmit={handleSubmit}>
          {[['Name', 'name', 'text'], ['Email', 'email', 'email'], ['Phone', 'phone', 'tel'], ['Password', 'password', 'password']].map(([label, key, type]) => (
            <div key={key} style={s.field}>
              <label style={s.label}>{label}</label>
              <input style={s.input} type={type} required value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} />
            </div>
          ))}
          <button style={s.btn} type="submit">Register</button>
        </form>
        <p style={s.link}>Already registered? <Link to="/delivery/login">Login</Link></p>
      </div>
    </div>
  );
}
